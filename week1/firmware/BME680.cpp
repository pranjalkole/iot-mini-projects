...
const uint8_t BME680_I2C_MAX_ADDRESS{
    0x77}; ///< Minimum possible address for BME680
...
const uint8_t BME680_ADDR_RES_HEAT_RANGE_ADDR{
    0x02};                              ///< Register for gas calibration
const uint8_t BME680_RHRANGE_MSK{0x30}; ///< Register for gas calibration
const uint8_t BME680_ADDR_RES_HEAT_VAL_ADDR{
    0x00}; ///< Register for gas calibration
const uint8_t BME680_ADDR_RANGE_SW_ERR_ADDR{
    0x04};                              ///< Register for gas calibration
...
/***********************************************************************************************
** The "begin()" function is overloaded to allow for variations in I2C and SPI
* initialization **
***********************************************************************************************/
bool BME680_Class::begin() {
    /*!
    * @brief   starts communications with device (overloaded)
    * @details When called with no parameters the I2C protocol with
    I2C_STANDARD_MODE speed is selected. The I2C network is searched for the
    first BME680 device.
    * return "true" if successful otherwise false
    */
    return begin(
        I2C_STANDARD_MODE,
        0); // Initialize I2C with standard speed, allow any i2c address
} // of method begin()
bool BME680_Class::begin(const uint32_t i2cSpeed) {
    /*!
    @brief   starts communications with device (overloaded)
    @details When called with a 32-bit parameter it is assumed that the I2C
    protocol is to be used and the speed setting is determined by the parameter.
    @param[in] i2cSpeed I2C Speed value
    return "true" if successful otherwise false
    */
    return begin(i2cSpeed,
                 0); // Initialize I2C with given speed, allow any i2c address
} // of method begin()
bool BME680_Class::begin(const uint32_t i2cSpeed, const uint8_t i2cAddress) {
    ...
    @details When called with a 32-bit parameter it is assumed that the I2C
    protocol is to be used and the speed setting is determined by the parameter.
    If the i2cAddress is 0 then check both, otherwise check only only the
    specified address
    @param[in] i2cSpeed I2C Speed value
    @param[in] i2cAddress I2C Address, use 0 to self-determine
    return "true" if successful otherwise false
    */
    wire.begin(DEFAULT_BME680_SDA, DEFAULT_BME680_SCL,
               i2cSpeed);    // Start I2C as master
    wire.setClock(i2cSpeed); // and set bus speed
    _I2CSpeed = i2cSpeed;
    for (_I2CAddress = BME680_I2C_MIN_ADDRESS;
         _I2CAddress <= BME680_I2C_MAX_ADDRESS;
         _I2CAddress++) { // Loop through all I2C addresses
        if (i2cAddress == 0 ||
            _I2CAddress == i2cAddress) {         // Only check if relevant
            wire.beginTransmission(_I2CAddress); // Check for BME680 here
            if (wire.endTransmission() ==
                0) { // We have found a device that could be
                return commonInitialization(); // a BME680, so perform common
                                               // init
...
bool BME680_Class::begin(const uint8_t chipSelect) {
    /*!
    @brief     starts communications with device (overloaded)
    @details   When called with a single 8-bit parameter is assumed that
    hardware SPI is to be used if the value is not 0x76 or 0x77, in which case
    it is assumed that it is the I2C addr
    @param[in] chipSelect Arduino Pin number for the Slave-Select pin or the I2C
    address return     "true" if successful otherwise false
    */
    if (chipSelect == BME680_I2C_MIN_ADDRESS ||
        chipSelect == BME680_I2C_MAX_ADDRESS) {
        // If 0x76 or 0x77 then we have I2C
        return begin(I2C_STANDARD_MODE,
                     chipSelect); // Std speed with explicit address
...
bool BME680_Class::begin(const uint8_t chipSelect, const uint8_t mosi,
...
bool BME680_Class::commonInitialization() {
    ...
    @details Called from all of the overloaded "begin()" functions once the
    communications protocol has been selected return "true" if successful
    otherwise false
    */
    uint8_t SPI_Register = readByte(BME680_SPI_REGISTER); // Read SPI reg. byte
    if (_I2CAddress == 0 &&
        bitRead(SPI_Register,
                BME680_SPI_MEM_PAGE_POSITION)) { // Wrong mode for ID
        bitWrite(SPI_Register, BME680_SPI_MEM_PAGE_POSITION,
                 0); // Turn off Page bit to go to Page 0
        putData(wire, BME680_SPI_REGISTER,
                SPI_Register); // Write value to register
    } // of if-then we are in SPI mode
    if (readByte(BME680_CHIPID_REGISTER) ==
        BME680_CHIPID) {        // check for correct chip id
        getCalibration();       // get the calibration values
        if (_I2CAddress == 0) { // If using SPI, switch to correct
            bitWrite(SPI_Register, BME680_SPI_MEM_PAGE_POSITION,
                     1); // Page "1" again
            putData(wire, BME680_SPI_REGISTER,
                    SPI_Register); // Update register value
        } // of if-then SPI mode
        uint8_t workRegister = readByte(
            BME680_CONTROL_MEASURE_REGISTER); // Read the control measure
        putData(wire, BME680_CONTROL_MEASURE_REGISTER,
...
uint8_t BME680_Class::readByte(const uint8_t addr) {
    ...
    @details interlude function to the "getData()" function. This is called so
    that the return value is the byte of data that has just been read rather
    than the number of bytes read returned by "getData()" param[in] addr Address
    of device return    single byte of read from addr
    */
    uint8_t returnValue;              // Storage for returned value
    getData(wire, addr, returnValue); // Read just one byte
...
void BME680_Class::reset() {
     ...
     */
    putData(wire, BME680_SOFTRESET_REGISTER,
            BME680_RESET_CODE); // write reset code to device
...
void BME680_Class::getCalibration() {
    /*!
    @brief    reads the calibration register data into local variables for use
    in converting readings
    @details  The calibration registers are read into a temporary array and then
    parsed into the appropriate calibration variables, this was taken from the
    example BOSCH software and minimizes register reads, but makes it rather
    difficult to read. This will be redone for legibility at some point in the
    future param[in] addr Address of device return    single byte read
    */
    /*************************************
    ** Temperature related coefficients **
    *************************************/
    uint8_t coeff_arr1[BME680_COEFF_SIZE1] = {
        0}; // Define temp array 1 and initialize with 0x
    uint8_t coeff_arr2[BME680_COEFF_SIZE2] = {
        0}; // Define temp array 2 and initialize with 0s
    getData(wire, BME680_COEFF_START_ADDRESS1,
            coeff_arr1); // Split reading registers into 2
    getData(wire, BME680_COEFF_START_ADDRESS2,
            coeff_arr2); // one 25 bytes and the other 16
    ...
    **********************************/
    _H1 = (uint16_t)(((uint16_t)coeff_arr2[BME680_H1_MSB_REG]
                      << BME680_HUM_REG_SHIFT_VAL) |
                     ((coeff_arr2[BME680_H1_LSB_REG] >>
                       BME680_HUM_REG_SHIFT_VAL) &
                      BME680_BIT_H1_DATA_MSK));
    _H2 = (uint16_t)(((uint16_t)coeff_arr2[BME680_H2_MSB_REG]
                      << BME680_HUM_REG_SHIFT_VAL) |
                     ((coeff_arr2[BME680_H2_LSB_REG] >>
                       BME680_HUM_REG_SHIFT_VAL) &
                      BME680_BIT_H1_DATA_MSK));
    ...
    ************************************/
    ...
    uint8_t temp_var = 0;
    getData(wire, BME680_ADDR_RES_HEAT_RANGE_ADDR, temp_var);
    _res_heat_range = ((temp_var & BME680_RHRANGE_MSK) / 16);
    getData(wire, BME680_ADDR_RES_HEAT_VAL_ADDR, temp_var);
    _res_heat = (int8_t)temp_var;
    getData(wire, BME680_ADDR_RANGE_SW_ERR_ADDR, temp_var);
...
uint8_t BME680_Class::setOversampling(const uint8_t sensor,
    ...
    @details See enumerated "sensorTypes" for a list of values. Set to a valid
    oversampling rate as defined in the enumerated type oversamplingTypes. If
    either value is out of range or another error occurs then the return value
    is false. param[in] sensor Enumerated sensor type param[in] sampling
    Sampling rate from enumerated type return "true" if successful otherwise
    false
    */
    if (sensor >= UnknownSensor ||
        (sampling != UINT8_MAX &&
         sampling >= UnknownOversample)) { // return an error if sensor or
                                           // oversample is out of range
        return (UINT8_MAX);
    }
    ...
    {
        tempRegister = readByte(
            BME680_CONTROL_HUMIDITY_REGISTER); // Read the register contents
        if (sampling == UINT8_MAX) {           // If we just want to read values

            returnValue =
                tempRegister & ~BME680_HUMIDITY_MASK; // Set return value
        } else {
            tempRegister &= BME680_HUMIDITY_MASK; // Mask bits to 0
            tempRegister |= sampling;             // Add in the sampling bits
            putData(wire, BME680_CONTROL_HUMIDITY_REGISTER,
    ...
    case TemperatureSensor: // Set the temperature oversampling
    {
        tempRegister = readByte(
            BME680_CONTROL_MEASURE_REGISTER); // Read the register contents
        if (sampling == UINT8_MAX) { // If we just want to read the values
            returnValue = (tempRegister & ~BME680_TEMPERATURE_MASK) >>
                          5; // Set return value
        } else {
            tempRegister &= BME680_TEMPERATURE_MASK; // Mask bits to 0
            tempRegister |=
                (sampling << 5); // Add in the sampling bits at offset
            putData(wire, BME680_CONTROL_MEASURE_REGISTER,
...
uint8_t BME680_Class::setIIRFilter(const uint8_t iirFilterSetting) {
    /*!
     ...
     @details When called with no parameters returns the current IIR Filter
     setting, otherwise when called with one parameter will set the IIR filter
     value and return the new setting param[in] iirFilterSetting New setting
     return   IIR Filter setting
     */
    waitForReadings(); // Ensure any active reading is finished
    uint8_t returnValue =
        readByte(BME680_CONFIG_REGISTER); // Get control register byte contents
    if (iirFilterSetting != UINT8_MAX)    // If the value is to be changed
    {                                     //
        returnValue = returnValue & B11100011; // mask IIR bits
        returnValue |= (iirFilterSetting & B00000111)
                       << 2; // use 3 bits of iirFilterSetting
        putData(wire, BME680_CONFIG_REGISTER,
                returnValue); // Write new control register value
    } // if the value is to be changed                                   //
    returnValue = (returnValue >> 2) &
                  B00000111; // Extract IIR filter setting from register
...
uint8_t BME680_Class::getSensorData(int32_t &temp, int32_t &hum, int32_t &press,
    ...
    /*!
     @brief   Returns the most recent temperature, humidity and pressure
     readings param[out] temp       Temperature reading param[out] hum Humidity
     reading param[out] press      Pressure reading param[out] gas        Gas
     reading param[in]  waitSwitch (Optional) When set will not return until
     reading is finished
     */
    uint8_t status =
...
uint8_t BME680_Class::readSensors(const bool waitSwitch) {
    /*!
     @brief   reads all 4 sensor values from the registers in one operation and
     then proceeds to convert the raw temperature, pressure & humidity readings
     into standard metric units
     @details The formula is written in the BME680's documentation but the math
     used below was taken from Adafruit's Adafruit_BME680_Library at
              https://github.com/adafruit/Adafruit_BME680. I think it can be
     refactored into more efficient code at some point in the future, but it
     does work correctly. param[in] waitSwitch (Optional) When set will not
     return until reading is finished
     */
    ...
    getData(wire, BME680_STATUS_REGISTER, buff); // read all 15 bytes in one go
    adc_pres =
        (uint32_t)(((uint32_t)buff[2] << 12) | ((uint32_t)buff[3] << 4) |
                   ((uint32_t)buff[4] >> 4)); // put the 3 bytes of Pressure
    adc_temp =
        (uint32_t)(((uint32_t)buff[5] << 12) | ((uint32_t)buff[6] << 4) |
                   ((uint32_t)buff[7] >> 4)); // put the 3 bytes of Temperature
    adc_hum = (uint16_t)(((uint32_t)buff[8] << 8) |
                         (uint32_t)buff[9]); // put the 2 bytes of Humidity
    adc_gas_res =
        (uint16_t)((uint32_t)buff[13] << 2 |
                   (((uint32_t)buff[14]) >> 6)); // put the 2 bytes of Gas
    gas_range = buff[14] & 0X0F;                 // Retrieve the range
                                 //*******************************//
                                 // First compute the temperature //
                                 //*******************************//
    var1 = ((int32_t)adc_temp >> 3) -
           ((int32_t)_T1 << 1);               // Perform calibration/adjustment
    ...
    return (buff[14] &
            0X30); // Return nonzero if gas or heat stabilization is invalid
...
bool BME680_Class::setGas(uint16_t GasTemp, uint16_t GasMillis) {
    ...
    waitForReadings(); // Ensure any active reading is finished
    uint8_t gasRegister =
        readByte(BME680_CONTROL_GAS_REGISTER2); // Read current register values
    if (GasTemp == 0 || GasMillis == 0) {
        // If either input variable is zero //
        putData(wire, BME680_CONTROL_GAS_REGISTER1,
                (uint8_t)B00001000); // Turn off gas heater
        putData(
            wire, BME680_CONTROL_GAS_REGISTER2,
            (uint8_t)(gasRegister & B11101111)); // Turn off gas measurements
    } else {
        putData(wire, BME680_CONTROL_GAS_REGISTER1,
                (uint8_t)0); // Turn off heater bit to turn on
        uint8_t heatr_res;
        ...
        var2 =
            (_G1 + 784) * (((((_G2 + 154009) * GasTemp * 5) / 100) + 3276800) /
                           10); // Issue #26
        var3 = var1 + (var2 / 2);
        ...
        putData(wire, BME680_GAS_HEATER_REGISTER0, heatr_res);
        uint8_t factor = 0;
        uint8_t durval;
        if (GasMillis >= 0xfc0)
        ...
        putData(wire, BME680_CONTROL_GAS_REGISTER1,
                (uint8_t)0); // then turn off gas heater
        putData(wire, BME680_GAS_DURATION_REGISTER0, durval);
        putData(wire, BME680_CONTROL_GAS_REGISTER2,
                (uint8_t)(gasRegister | B00010000));
...
bool BME680_Class::measuring() {
     ...
     */
    bool result(false);
    if ((readByte(BME680_STATUS_REGISTER) &
...
void BME680_Class::triggerMeasurement() {
     ...
     */
    uint8_t workRegister =
        readByte(BME680_CONTROL_MEASURE_REGISTER); // Read the control measure
    putData(wire, BME680_CONTROL_MEASURE_REGISTER,
